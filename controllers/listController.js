...
exports.updateList = catchAsync(async (req, res, next) => {
    if (req.body.user || req.body.coverPhoto) {
        return next(
            new AppError(
                401,
                `You can't change the user that the list belongs to, or the coverPhoto directly from the API.`
            )
        );
    }
    if (req.body.tasks) {
        return next(
            new AppError(
                401,
                'To add, update and remove tasks, use /api/v1/lists/id/tasks/taskID '
            )
        );
    }
...
exports.handleViewPost = catchAsync(async (req, res, next) => {
    if (req.body.createTask) {
        ...
        for (let task in req.body) {
            ...
            req.list.tasks[index].done = Array.isArray(req.body[task])
                ? true
                : false;
            await req.list.save();
        }

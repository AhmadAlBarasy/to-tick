...
exports.login = catchAsync(async (req, res, next) => {
    ...
    const user = await User.findOne({ email: email }).select(
        '+password -profilePicture -__v -__id'
    );
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!token) {
        return next(
            new AppError(
                403,
                'You are not authenticated, please sign up or login to do this action.'
            )
        );
    }
    ...
    if (!user) {
        return next(
            new AppError(403, 'Something wrong happened, please login again.')
        );
    }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    const resetUrl = req.originalUrl.startsWith('/api')
